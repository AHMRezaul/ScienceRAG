...
def search_and_download_pmc_pdfs(query, max_results=3, save_directory="downloaded_pdfs"):
    ...
    for pmc_id in pmc_ids:
        try:
            ...
            print(f"Error fetching PDF for PMC ID {pmc_id}: {e}")
            continue
...
def read_pdf(file_path):
    ...
    with open(file_path, "rb") as f:
        ...
        for page in reader.pages:
            ...
            if page_text and "references" not in page_text.lower():
                text += page_text
        return text
...
def call_gpt_for_combined_context(query, combined_context):
    ...
    try:
        messages = [
            ...
            {
                "role": "user",
                "content": f"Based on the following content, answer the query clearly and concisely. The answer should be short.\n\nContent:\n{combined_context}\n\nQuery: {query}\n\nAnswer:",
            },
        ]
...
def main():
    ...
    while True:
        ...
        #query = input(input_prompt.rjust(terminal_width))
